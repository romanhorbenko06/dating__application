...
/**
 * Читання тега орієнтації (EXIF 0x0112) з JPEG.
 *
 * Навіщо: телефон майже завжди пише кадр на матрицю в одній і тій самій орієнтації,
 * а те, як його повертати, кладе окремим тегом. Якщо просто зрізати метадані
 * (а ми це робимо навмисно — там же лежать GPS-координати), знімок втратить
 * інструкцію повороту й ляже набік. Тому орієнтацію читаємо ДО зачистки
 * і застосовуємо до пікселів — далі вона вже не потрібна нікому.
 *
 * Розбираємо вручну, без сторонніх бібліотек: потрібен рівно один тег.
 */
